...
from model.transformer_model_s2s_unweight import TransformerUnweightModel
from model.trainer_s2s_unweight import TrainerUnweight
from model.text import myVocab
from model.dataset import SMPDataset_soft
from config import get_model_config_unweight, get_trainer_config_unweight
...
def main():
    model_config = get_model_config_unweight()
    trainer_config = get_trainer_config_unweight()
    ...
    transformer =  TransformerUnweightModel(n_layers=model_config.n_layers,
    ...
    train_dataset = SMPDataset_soft(trainer_config.train_datasets, vocab, transformer.n_pos_embeddings - 1)
    test_dataset = SMPDataset_soft(trainer_config.test_datasets, vocab, transformer.n_pos_embeddings - 1)

    model_trainer =  TrainerUnweight(transformer,
                                train_dataset,
                                test_dataset,
    ...
    if distributed:
        ...
        # model_trainer.model.cls_linear = DistributedDataParallel(model_trainer.model.cls_linear, device_ids=[args.local_rank], output_device=args.local_rank)
    ...
    try:
        if args.local_rank in [-1, 0]:
            ...
            model_trainer.train(start_epoch, trainer_config.n_epochs, after_epoch_funcs=[save_func, sample_text_func],
                                risk_func=f1_risk)
        else:
