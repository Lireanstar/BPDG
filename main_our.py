...
from config import get_model_config_our, get_test_config_our
...
class Model:
    ...
    def __init__(self):
        ...
        test_config = get_test_config_our()
        ...
        # test_config.last_checkpoint_path = r'E:\peasona_chatbot\smp_zhangrongsheng\checkpoints\last_checkpoint490'
    ...
    def next_word_probability(self, context, partial_out, weight_i=None, human=False):
        """
    ...
    def prepare_context(self, contexts):
        for context in contexts:
            ...
            with torch.no_grad():
                ...
                # weight = self.transformer.compute_weight(enc_contexts[1])
                weight = weight_i
                return enc_contexts, weight

    def gen_response(self, contexts, weight_i=None, human=False):
        """
        ...
        ==>  [['I', 'am', 'fine', 'too', '!']]
        """
        greedy_res, sample_res, topk_res, topp_res = [], [], [], []
        for context in contexts:
            if 'responder_profile' in context:
                responder_profile = context['responder_profile']
                tag = responder_profile['tag'].replace(' ', '')
            ...
            with torch.no_grad():
                ...
                topp_prediction = self.transformer.top_p(
                    enc_contexts, weight=weight, sample_count=10, p=0.8, temperature=1.0)[0]
                greedy_prediction = self.transformer.greedy(enc_contexts, weight=weight)[0]
                sample_prediction = self.transformer.sample_resp(
                    enc_contexts, weight=weight, sample_count=10, temperature=1.0)[0]
                sample_len = [i[1:].index(self.vocab.eos_id) + 1 for i in sample_prediction]
                ppl = self.transformer.cal_ppl(sample_prediction, sample_len, enc_contexts)
                print(ppl)
                topk_prediction = self.transformer.top_k(
                    enc_contexts, weight=weight, sample_count=10, k=10, temperature=1.0)[0]

            greedy_prediction = self.vocab.ids2string_wo_eos(greedy_prediction)
            sample_prediction = [self.vocab.ids2string_wo_eos(i) for i in sample_prediction]
            topk_prediction = [self.vocab.ids2string_wo_eos(i) for i in topk_prediction]
            topp_prediction = [self.vocab.ids2string_wo_eos(i) for i in topp_prediction]
            greedy_res.append(greedy_prediction)
            sample_res.append(sample_prediction)
            topk_res.append(topk_prediction)
            topp_res.append(topp_prediction)

        return greedy_res, sample_res, topk_res, topp_res


def test_biased(model):
    with open('data/test_data_biased_our.txt', 'w', encoding='utf8') as fw:
        with open('data/test_data_biased.json', 'r', encoding='utf8') as fr:
            ...
            for line in lines:
                ...
                ans_no_persona = model.gen_response([data], weight_i=weights[1])
                ans_no_persona = ''.join(ans_no_persona[0])
                fw.write('predict with auto weight: ' + ans_auto + '\n')
                fw.write('predict with full persona: ' + ans_persona + '\n')
                fw.write('predict with no persona: ' + ans_no_persona + '\n')
                fw.write('\n')


def test_random(model):
    with open('data/test_data_random_our.txt', 'w', encoding='utf8') as fw:
...
if __name__ == '__main__':
    ...
    test_data = './data/test_data_biased.json'
    with codecs.open(test_data, 'r', 'utf-8') as f:
        contents = [i.strip() for i in f.readlines() if len(i.strip()) != 0]
    test_data = [json.loads(i) for i in contents][:100]

    for count, content in enumerate(test_data):
        print('Count: {}'.format(count))
        for i in range(len(content['dialog'])):
            print('s{}: {}'.format(content['uid'][i], content['dialog'][i][0]))
        for i in range(len(content['profile'])):
            print('profile for s{}: {}'.format(content['uid'][i], content['profile'][i]))

        print('Responder: {}'.format(content['responder_profile']))

        greedy_resp, sample_resp, topk_resp, topp_resp = model.gen_response([content])
        print('Greedy Response: {}'.format(greedy_resp[0]))
        print('Sample Response:')
        for i, utt in enumerate(sample_resp[0]):
            print('{}: {}'.format(i, utt))

        print('TopK Response:')
        for i, utt in enumerate(topk_resp[0]):
            print('{}: {}'.format(i, utt))

        print('TopP Response:')
        for i, utt in enumerate(topp_resp[0]):
            print('{}: {}'.format(i, utt))
