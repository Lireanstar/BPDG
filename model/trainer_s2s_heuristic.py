...
import json
import logging
logger = logging.getLogger('s2s-our-heuristic')
logger.setLevel(logging.INFO)
fh = logging.FileHandler('s2s-our-heuristic.log', encoding='utf-8')
...
class TrainerHeuristic:
    ...
    def collate_func(self, data):
        ...
        weight = [[0,1] if d else [1,0] for d in label]
        weight = torch.Tensor(weight)
        label = torch.LongTensor(label)
        return contexts, y, gender_cat, loc_cat, tag_cat, label, weight
    ...
    def _eval_train(self, epoch, risk_func=None):
        ...
        lm_loss = 0
        # cls_loss = 0
        for i, (contexts, targets, gender, loc, tag, label, weight) in enumerate(tqdm_data):
            ...
            outputs = self.model.decode(prevs, enc_contexts, weight=weight)
            ...
            tqdm_data.set_postfix({'lm_loss': lm_loss, 'loss': loss, 'loss_step': batch_loss.item(),
                                   'lr': self.optimizer.rate(), 'step': self.optimizer._step})

        log_dict = {'epoch': epoch, 'lm_loss': lm_loss, 'loss': loss,
                    'lr': self.optimizer.rate(), 'step': self.optimizer._step}
    ...
    def _eval_test(self, metric_funcs={}):
        ...
        for i, (contexts, targets, gender, loc, tag, label, weight) in enumerate(tqdm_data):
            ...
            predictions = self.model.beam_search(enc_contexts, weight=None)
