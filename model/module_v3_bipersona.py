        ...
        if weight is None:
            ...
            for i in range(0, len(inputs), 2): ##source, target, history
                if i == 0:
                    ...
                    a = weight[:, 0].view(-1, 1, 1) * a
                if i == 2:
                    c2, m2 = inputs[2], inputs[3].bool()  ##contains 2 parts ,
                    a = self.attn(x, c2, c2, m2)  # target mask
                    a = weight[:, 1].view(-1, 1, 1) * a
                if i ==4 :## Oc
                    c, m = inputs[4], inputs[5].byte()  ##contains 2 parts , one is the persona_info two is the history
...
class TransformerModule(nn.Module):
    def __init__(self, n_layers, n_embeddings, n_pos_embeddings, embeddings_size,
                 ...
                 n_segments=None, n_gender=3, n_loc=5, n_tag=502):
