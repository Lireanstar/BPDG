...
from model.trainer_s2s_heuristic import TrainerHeuristic
from model.text import myVocab
from model.dataset import SMPDataset_our
from config import get_model_config_heuristic, get_trainer_config_heuristic
...
def main():
    model_config = get_model_config_heuristic()
    trainer_config = get_trainer_config_heuristic()
    ...
    if not trainer_config.load_last:
        ...
        # transformer.transformer_module.load_state_dict(openai_model, strict=False)
    ...
    model_trainer = TrainerHeuristic(transformer,
                                     train_dataset,
                                     test_dataset,
    ...
    def sample_text_func(epoch):
        ...
        for persona, dialog, target, gender, loc, tag, label in samples:
            ...
            # weight = None
            if label:
                weight = torch.Tensor([[0, 1]]).to('cuda')
            else:
