...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, padding_mask):
        ...
        if qkv_same:
            ...
            apply_future_mask = False #unmasked !!!
        else:
            assert False
...
class TransformerBlock(nn.Module):  ##standard module
    ...
    def forward(self, x, padding_mask, *contexts, weight=None):
        ...
        if weight is None:
            ...
            full_attn = self.attn(x, x, x, padding_mask.byte())  # batch_size, max_seqlen, hidden_size masked self-attention
            inputs = contexts
            temp_attn = 0
            for i in range(0, len(inputs), 2): ##source, target, history
                if i == 0:
                    c1, m1 = inputs[0], inputs[1].bool()  ##contains 2 parts , one is the persona_info two is the history
                    a = self.attn(x, c1, c1, m1)  # source mask
                    a = a* 0.33
                if i == 2:
                    c2, m2 = inputs[2], inputs[3].bool()  ##contains 2 parts , one is the persona_info two is the history
                    a = self.attn(x, c2, c2, m2)  # target mask
                    a = a* 0.33
                if i ==4 :## Oc
                    c, m = inputs[0], inputs[1].byte()  ##contains 2 parts , one is the persona_info two is the history
                    temp = self.attn(x, c, c, m)  ##unmasked bi-directional self-attention [batch_size, max_seq, embbeded_size]
...
class TransformerModule(nn.Module):
    def __init__(self, n_layers, n_embeddings, n_pos_embeddings, embeddings_size,
                 ...
                 n_segments=None, n_gender=3, n_age=5, n_tag=502):
        ...
        self.age_embeddings = nn.Embedding(n_age, embeddings_size, padding_idx=0)  ##37 是统计的数目, 有多少个就embbeded 几个
    ...
    def _init_weights(self):
        ...
        nn.init.normal_(self.age_embeddings.weight, std=0.02)
        nn.init.normal_(self.tag_embeddings.weight, std=0.02)

    def forward(self, x, enc_contexts=[], gender=None, age=None, tag=None, weight=None):
        ...
        if age is not None:
            x = x + self.age_embeddings(age)
        if tag is not None:
            x = x + self.tag_embeddings(tag)  ##这里可以加人脸识别的特征，抽取的特征 + 再加入一个,或者在后面？
        x = self.embed_dropout(x)

        enc_contexts = sum(enc_contexts, ())  ##当 s2s 的时候 就有enc_contextd 现在 有6个，加了双方个性化的特征
