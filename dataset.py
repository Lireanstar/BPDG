...
class SMPDataset_soft(Dataset):
    ...
    def make_dataset(self, paths, vocab, max_lengths):
        ...
        with open(paths, 'r', encoding='utf8') as fr:
            lines = fr.readlines()
            for line in tqdm.tqdm(lines[:50000]):
...
class SMPDataset_our(Dataset):
    ...
    def make_dataset(self, paths, vocab, max_lengths):
        ...
        with open(paths, 'r', encoding='utf8') as fr:
            ...
            for line in tqdm.tqdm(lines):
                ...
                profile = data['profile']
                uid = data['uid']
                age_tag = {'00后': 1, '90后': 2, '80后': 3, '70后': 4}
                age_str = {'1': '00后', '90': '90后', '80': '80后', '70': '70后'}
                if len(dialog) == len(uid) and len(dialog) > 1:
                    dialog_ids = [vocab.string2ids(' '.join(i[0].replace(' ', ''))) for i in dialog]
                    profile_ids = []
                    profile_embedding_ids = []
                    for i in profile:
                        ...
                        persona = '性别:' + gender + ',' + '年龄:' + age + ',' + '标签:' + tag  ##还是以字符串的形式直接嵌入
                        profile_ids.append(vocab.string2ids(' '.join(persona)))
    ...
    def __getitem__(self, idx):
        ...
        history_cat = [self.vocab.eos_id]
        gender_cat = [profile_embedding_ids[uid[0]][0]]
        age_cat = [profile_embedding_ids[uid[0]][1]]  # break 的作用 就是 做成三元组的形式方便这里选出来
        ...
        return source_profile, target_profile, history_cat, response, gender_cat, age_cat, tag_cat, label
...
class SMPDataset_lost(Dataset):
    def __init__(self, paths, vocab, max_lengths=2048):
        ...
        self.vocab = vocab
        self.max_lengths = max_lengths
        self.data = SMPDataset_lost.make_dataset(paths[0], vocab, max_lengths)
...
if __name__ == '__main__':
    ...
    SMPdata = SMPDataset_our(trainer_config.train_datasets, vocab, 512)
