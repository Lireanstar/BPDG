...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, padding_mask, *contexts, weight=None):
        ...
        if weight is None:
            ...
            full_attn = self.attn(x, x, x, padding_mask.bool())
            inputs = contexts
            temp_attn = 0
            for i in range(0, len(inputs), 2):
                c, m = inputs[i], inputs[i + 1].bool()
                temp = self.attn(x, c, c, m)
                if i//2 == 1:
                ...
                temp_attn += a
            full_attn = (full_attn + temp_attn)/3
...
class TransformerModule(nn.Module):
    def __init__(self, n_layers, n_embeddings, n_pos_embeddings, embeddings_size,
                 ...
                 n_segments=None, n_gender=None, n_loc=None, n_tag=None):
        ...
        if n_gender is not None:
            self.gender_embeddings = nn.Embedding(n_gender, embeddings_size, padding_idx=0)
        if n_loc is not None:
            self.loc_embeddings = nn.Embedding(n_loc, embeddings_size, padding_idx=0)
        if n_tag is not None:
            self.tag_embeddings = nn.Embedding(n_tag, embeddings_size, padding_idx=0)
