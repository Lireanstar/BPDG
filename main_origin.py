...
from config import get_model_config_origin, get_test_config_origin
...
class Model:
    ...
    def __init__(self):
        ...
        test_config = get_test_config_origin()
    ...
    def next_word_probability(self, context, partial_out):
        ...
        contexts = [torch.tensor([c], dtype=torch.long, device=self.device) for c in [history_cat] if len(c) > 0]
        with torch.no_grad():
    ...
    def gen_response(self, contexts):
        ...
        for context in contexts:
            ...
            profile = profile[:48]
            history_cat = history_cat[-128:]
            contexts = [torch.tensor([c], dtype=torch.long, device=self.device) for c in [history_cat] if
                        len(c) > 0]
            with torch.no_grad():
...
if __name__ == '__main__':
    model = Model()
    # test_biased(model)
    files = [['data/test_data_biased.json', 'data/test_data_biased_seq2seq.txt'],
             ['data/test_data_random.json', 'data/test_data_random_seq2seq.txt']]
