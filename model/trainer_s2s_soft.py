...
logger = logging.getLogger('s2s-our')
logger.setLevel(logging.INFO)
fh = logging.FileHandler('s2s-our-aaai-2020.log', encoding='utf-8')
...
class TrainerSoft:
    ...
    def collate_func(self, data):
        ...
        if max(map(len, persona_info)) > 0:
            ...
            persona_info = pad_sequence(persona_info, batch_first=True,
        ...
        label = torch.LongTensor(label)  ##cls target
    ...
    def _eval_train(self, epoch, risk_func=None):
        ...
        for i, (contexts, targets, gender, loc, tag, label) in enumerate(tqdm_data):
            ...
            if self.lm_weight > 0:  ##lm_head : 针对histroy 而言
                ...
                prevs, nexts = context_outputs[:, :-1, :].contiguous(), contexts[1][:,
