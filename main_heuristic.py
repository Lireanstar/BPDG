...
from config import get_model_config_heuristic, get_test_config_heuristic
...
class Model:
    ...
    def __init__(self):
        ...
        test_config = get_test_config_heuristic()
    ...
    def next_word_probability(self, context, partial_out, weight_i=None):
        ...
        weight_i = torch.Tensor([[0.5, 0.5]]).to('cuda')
        ...
        with torch.no_grad():
            ...
            prediction = self.transformer.predict_next(enc_contexts, prefix=partial_out_ids, weight=weight_i)
    ...
    def gen_response(self, contexts, weight_i=None):
        ...
        for context in contexts:
            ...
            with torch.no_grad():
                ...
                dialog_enc = self.transformer.encode(contexts[1], gender=gender_format, loc=loc_format, tag=tag_format)
                enc_contexts = [persona_enc, dialog_enc]
                prediction = self.transformer.beam_search(enc_contexts, weight=weight_i)[0]
...
if __name__ == '__main__':
    model = Model()
    # test_biased(model)
    files = [['data/test_data_biased.json', 'data/test_data_biased_heuristic.txt'],
             ['data/test_data_random.json', 'data/test_data_random_heuristic.txt']]
