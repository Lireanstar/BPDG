...
from model.trainer_s2s_origin import TrainerOrigin
from model.text import myVocab
from model.dataset import SMPDataset_lost
from config import get_model_config_origin, get_trainer_config_origin
...
def main():
    model_config = get_model_config_origin()
    trainer_config = get_trainer_config_origin()
    ...
    train_dataset = SMPDataset_lost(trainer_config.train_datasets, vocab, transformer.n_pos_embeddings - 1,istest= False)
    test_dataset = SMPDataset_lost(trainer_config.test_datasets, vocab, transformer.n_pos_embeddings - 1,istest= True)

    model_trainer =  TrainerOrigin(transformer,
                                    train_dataset,
                                    test_dataset,
    ...
    def save_func(epoch):
        ...
        if os.path.exists(trainer_config.last_checkpoint_path + str(epoch-20)):
            os.remove(trainer_config.last_checkpoint_path + str(epoch-20))
    ...
    def sample_text_func(epoch):
        ...
        for context, dialog, target in samples:
            contexts = [torch.tensor([c], dtype=torch.long, device=model_trainer.device) for c in [dialog] if
                        len(c) > 0]
