...
from config import get_model_config_unpretrain, get_trainer_config_unpretrain
...
def main():
    ...
    trainer_config = get_trainer_config_unpretrain()
    ...
    if (not trainer_config.load_last) and pretrain:
    ...
    try:
        if args.local_rank in [-1, 0]:
            model_trainer.train(start_epoch, trainer_config.n_epochs,
                                after_epoch_funcs=[save_func, sample_text_func],
                                risk_func=f1_risk)
        else:
            model_trainer.train(start_epoch, trainer_config.n_epochs)
