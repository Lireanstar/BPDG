...
from config import get_model_config_unweight, get_test_config_unweight
...
class Model:
    ...
    def __init__(self):
        ...
        test_config = get_test_config_unweight()
    ...
    def next_word_probability(self, context, partial_out):
        ...
        with torch.no_grad():
            ...
            prediction = self.transformer.predict_next(enc_contexts, prefix=partial_out_ids, weight=None)
    ...
    def gen_response(self, contexts):
        ...
        for context in contexts:
            ...
            with torch.no_grad():
                ...
                dialog_enc = self.transformer.encode(contexts[1], gender=gender_format, loc=loc_format, tag=tag_format)
                enc_contexts = [persona_enc, dialog_enc]
                prediction = self.transformer.beam_search(enc_contexts, weight=None)[0]
    ...
    with open(output_file, 'w', encoding='utf8') as fw:
        with open(input_file, 'r', encoding='utf8') as fr:
            ...
            for line in lines[:10]:
                ...
                fw.write('predict with unweight: ' + ans_auto + '\n')
                fw.write('\n')

if __name__ == '__main__':
    model = Model()
    # test_biased(model)
    files = [['data/test_data_biased.json', 'data/test_data_biased_unweight.txt'],
             ['data/test_data_random.json', 'data/test_data_random_unweight.txt']]
